...
*/
	...
	* @Title: 比较两时间大小 
	* @param sourceDay  源日期
	* @param targetDay  待比较日期
	* @return long    返回类型：正负数或者0   0：两日期相等， 正数：源日期大于待比较日期，  负数：源日期小于待比较日期 
	* @author linjq
	* @date 2016年11月7日 上午10:29:03 
	* @throws
	 */
	public static long compareDays(String sourceDay, String targetDay) {
		SimpleDateFormat shortSf = new SimpleDateFormat(YearMonthDay);
		Date d1 = null;
		Date d2 = null;
		try {
			d1 = shortSf.parse(sourceDay);
			d2 = shortSf.parse(targetDay);
		} catch (ParseException e) {
			e.printStackTrace();
		}
	 ...
	 * @return
	 */
	public static Date setDateTime(Date date, String time) {
		String dateStr = formatDate(date, "yyyy-MM-dd");
		dateStr = dateStr + " " + time + ":00";
		return formateDate(dateStr);
	}
	
	/**
	 * 得到一个星期的结束时间
	 * @param date - 周日结束 周一开始周日结束，所以往下推一周
	 * @return
	 */
	...
	public static void main(String[] args) {
		...
		System.out.println(getWeekDate(new Date(), 1));
	}
